...
function myclick(perso){
    ...
    if(pick[currentPosition]){
        $("#img_pick"+currentPick).fadeOut(0, function(){
            $("#img_pick"+currentPick).attr('src', 'res/'+id+'_carte.jpg');
          }).fadeIn(300);
        $("#"+id+" .pick-img").hide().fadeIn(600);
        cpt_pick++;
    }else{
        $("#"+id+" .ban-img").fadeIn(300);
        $("#img_ban"+currentBan).attr('src', 'res/'+id+'_ban.jpg');
        cpt_ban++;
    }
...
function reClick(perso){
  ...
  if(pick[indexPerso]){
    ...
    $("#img_pick"+pickPosition).fadeOut(0, function(){
        $("#img_pick"+pickPosition).attr('src', 'res/cache.png');
      }).fadeIn(300);
      $("#"+id+" .pick-img").hide().fadeOut(600);
      cpt_pick--;
  }else{
    const banPosition = getBanPosition(indexPerso);
    $("#img_ban"+banPosition).attr('src', 'res/basic_ban.png');
...
function back(){
    ...
    if(pick[cpt]){
        ...
        $("#img_pick"+cpt_pick).attr('src', 'res/cache.png');
...
function reset(){
    ...
    for (var i = 0; i < 6; i++) {
      $("#img_pick"+i).attr('src', 'res/cache.png');
    }
}
